...
| --- | --- | --- | --- | --- |
| `/` | Phase 0 reference audit completed on 2026-03-30 UTC. | The public shell still needs a stricter near-match for hero-to-card proportions, setup-panel asymmetry, and footer spacing at both desktop and mobile breakpoints. | Keep signed-in redirect behavior, tenant-aware selection, self-hosted collapse rules, and truthful setup-pending prominence. | Extract the public shell and access-status card primitives before refining spacing and typography against the landing reference. |
| `/console` | Phase 0 reference audit completed on 2026-03-30 UTC. | The dashboard still needs tighter bento proportions and clearer separation between the dominant feature card, utility cards, and recent-activity treatment. | Preserve role-sensitive navigation, truthful route links, and real counts instead of backfilling mock analytics. | Centralize dashboard card primitives and align the dashboard grid to the reference's feature-first hierarchy. |
