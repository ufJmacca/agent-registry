...
| `/tenants/:tenantId/environments` | `environment_management/code.html`; `environment_management/screen.png` | Authenticated shell, oversized page intro, layered surfaces, and editorial whitespace instead of list-plus-form compression. | The reference favors an inventory-dominant panel with creation framed as a secondary control surface rather than as a peer table action. | The existing environment inventory, truthful configured-target and creation-flow summaries, and the unchanged add-environment POST form replace cluster KPIs while preserving real tenant environment keys and creation behavior. | KPI cards, row overflow menus, export JSON, global logs, and other unsupported infrastructure utilities. |
...
| --- | --- | --- | --- | --- |
| `/` | Public sign-in fidelity pass completed on 2026-03-30 UTC. | The public shell now owns the anchored top bar, editorial hero, access/setup split, and footer rhythm, but the reference's extra marketing destinations and decorative utility tiles remain intentionally absent. | Keep signed-in redirect behavior, tenant-aware selection, self-hosted collapse rules, and truthful setup-pending prominence. | Closed for final sign-off; guard the approved public-shell composition through the comparison record and the committed desktop and mobile baselines. |
| `/console` | Dashboard fidelity pass completed on 2026-03-30 UTC. | The dashboard now follows the feature-first hierarchy, supporting utility panels, and lower register band from the reference, but the mock recent-activity strip and profile portrait remain intentionally absent because the product has no truthful activity feed or user-media source. | Preserve role-sensitive navigation, truthful route links, publisher-versus-admin visibility rules, and real version or active-agent counts instead of backfilling mock analytics. | Closed for final sign-off; guard the approved dashboard shell and card hierarchy through the comparison record and the committed desktop and mobile baselines. |
| `/tenants/:tenantId/environments` | Environment-management fidelity pass completed on 2026-03-30 UTC. | The route now uses a dedicated page module plus shared intro, record-list, form-field, and side-panel primitives, while the remaining fidelity gap is limited to intentional truthful substitutions where the reference's KPI ribbon and dense operations table are replaced by editorial summaries and card rows. | Keep the existing environment list, `environmentKey` submission behavior, admin-only access controls, and the `environment-list` masking hook intact. | Closed for final sign-off; regression-watch the inventory-primary and creation-secondary balance through the comparison record and committed baselines only. |
| `/tenants/:tenantId/drafts/new` | Draft-registration fidelity pass completed on 2026-03-30 UTC. | The route now uses shared form-section, publication-subpanel, and action-footer primitives for metadata, contracts, per-environment publications, and the footer CTA; the remaining gap is limited to screenshot-level spacing polish between the hero, upper grid, and footer at the widest desktop breakpoint. | Preserve every current field name, multipart behavior, publisher restriction, create-draft submission path, and the truthful no-environments state with no fake publication controls. | Closed for final sign-off; guard the approved grouped workflow, hero spacing, and footer rhythm through the comparison record and the committed desktop and mobile baselines. |
| `/tenants/:tenantId/review` | Review queue fidelity pass completed on 2026-03-30 UTC. | The queue now renders as curated review objects with immediate decision grouping, and the remaining fidelity gap is limited to the reference's richer machine-generated diagnostics and avatar/icon treatment that the current product cannot support truthfully. | Preserve tenant-admin-only access, the current approve and reject POST targets, version-detail linking, and the live reject `reason` field instead of inventing unsupported queue tooling. | Closed for final sign-off; guard the approved decision-first queue composition through the comparison record and the committed desktop and mobile baselines. |
| `/tenants/:tenantId/agents/:agentId` | Phase 2 agent-detail fidelity pass completed on 2026-03-30 UTC. | The remaining gap is limited to unsupported top-bar utility chrome, deploy/update controls, and mock uptime or token summaries that cannot be rendered truthfully. | Preserve truthful overlay state, active publication data, admin-only controls, and direct version-history navigation. | Closed for final sign-off; guard the approved dossier composition through the comparison record and the committed desktop and mobile baselines. |
| `/tenants/:tenantId/agents/:agentId/versions/:versionId` | Version-detail fidelity pass completed on 2026-03-30 UTC. | The route now uses a dedicated dossier wrapper, shared hero and section-frame primitives, a stronger review-state sidebar, audit history, and supporting metadata sections; the remaining gap is limited to intentionally omitted mock checklist badges, copy utilities, and fictional editorial facts. | Keep truthful review state, manifest content, submission and approval behavior, ownership restrictions, telemetry, health history, and documented deviations. | Closed for final sign-off; guard the approved dossier hierarchy and supporting mobile proof captures through the comparison record and committed baselines. |

## Final Audit Block

- Comparison record: [`FIDELITY_COMPARISON.md`](./FIDELITY_COMPARISON.md) covers all seven routes with direct links to each mapped reference pair, the approved desktop and mobile baselines, a side-by-side comparison summary, and the final residual-delta ledger.
- Screenshot inventory: `tests/visual/__screenshots__/` contains the approved `1440x1200` desktop and `390x844` mobile baselines for all seven routes plus the required `version-detail-mobile-proof-manifest.png` and `version-detail-mobile-proof-publications.png` proof captures.
- Validation path: Final sign-off is gated by `make lint`, `make test`, and `make migrate`, with `make test` fanning out to the committed unit suite and Playwright visual regression checks against the checked-in baselines.
- Residual delta disposition: Every recorded fidelity delta is now either `resolved` or an `approved truthful deviation`, and no open or provisional fidelity work remains in the sign-off state.
...
| `/tenants/:tenantId/environments` | `environment_management` | Cluster KPI cards, per-row overflow menus, export JSON, and global logs controls | Configured environment inventory, truthful configured-target and creation-flow summaries, and the existing add-environment form with the unchanged POST target | The route currently supports truthful environment listing and creation only, so the reference KPI and utility chrome is replaced with real state summaries instead of inert controls. |
| `/tenants/:tenantId/drafts/new` | `new_draft_registration` | Model picker, autosave messaging, save-as-draft / submit-for-review footer actions | Real draft metadata fields, shared contract JSON textareas, per-environment multipart publication panels, a truthful no-environments empty state without fake disabled publication controls, and a single create-draft submit action | The current workflow creates one draft version, then uses version detail for later submission; unsupported workflow controls were removed. |
...
| `/tenants/:tenantId/agents/:agentId` | `active_agent_detail` | Deploy-update CTA, analytics/deployments/settings rail items, add-custom-protocol affordance, and mock uptime or token KPI tiles | Dedicated dossier hero with real tenant, active-version, publication-count, and overlay-count facts; truthful publication panels, agent and environment overlay controls, and direct version history links | The route only supports truthful overlay mutations, publication facts, and dossier navigation; unsupported operations and synthetic metrics remain omitted. |
| `/tenants/:tenantId/agents/:agentId/versions/:versionId` | `version_detail` | Mock KPI cards, copy-JSON utility, review checklist badges, and side metadata such as license and production-ready labels | Publication contract cards, read-only manifest, review-state sidebar, audit history, supporting metadata, environment dossiers, telemetry, health history, and truthful action cluster | The current route exposes real review and publication data, but it does not store the reference's mock operational metrics or auxiliary utilities. |
...
- Review evidence: `code.html` and `screen.png` were compared against the current environment-management route, the dedicated `ui/pages/environment-management.ts` renderer, and the approved desktop and mobile baselines on 2026-03-30 UTC.
- Shell composition and overall layout: The page now uses the shared authenticated shell, shared section-frame primitives for the hero and inventory panel, a primary inventory surface, and a secondary creation rail that stack vertically on mobile without changing workflow order.
...
- CTA treatment and hierarchy: Add Environment remains the only submit CTA and stays visually secondary to the configured inventory panel, while truthful configured-target and creation-flow summaries replace the mock KPI ribbon.
- Card and background layering: Inventory and creation sit on separate layered surfaces with generous spacing and no hard separators, and the creation rail is composed from shared side-panel and form-section primitives instead of bespoke wrappers.
...
- Information density and grouping: Configured environments remain the primary information block as curated card rows, while the add-environment workflow is contained in its own technical side panel with supporting copy and unchanged field semantics.
...
- Intentional deviations and truthful substitutions: Mock uptime/load KPI cards were replaced with truthful configured-target and creation-flow summaries, while export controls, global logs utilities, and per-row overflow menus were omitted because the current product truthfully supports only environment listing and creation.

### `/tenants/:tenantId/drafts/new`

- Reference assets: `new_draft_registration/code.html`, `new_draft_registration/screen.png`
- Review evidence: `code.html` and `screen.png` were compared against the current draft-registration route, the shared form primitive outputs, and the approved desktop and mobile baselines on 2026-03-30 UTC.
...
- Card and background layering: Metadata, shared contracts, per-environment publication panels, and the footer CTA are composed through the shared form-section, form-subpanel, and form-action-footer primitives so the route uses consistent surfaces rather than one-off wrappers.
...
- Functional constraints to preserve: All existing field names, multipart uploads, POST target, publisher permissions, `data-visual-dynamic="publication-sections"`, and per-environment publication semantics remain unchanged.
- Intentional deviations and truthful substitutions: Mock model selection, autosave language, and save/submit dual actions were replaced with the real draft fields and the current create-then-review workflow already enforced by the app; when no environments exist, the route now renders a truthful empty state instead of inventing disabled publication controls.

### `/tenants/:tenantId/review`

- Reference assets: `review_queue/code.html`, `review_queue/screen.png`
- Review evidence: `code.html` and `screen.png` were compared against the updated review-queue route plus the approved desktop and mobile baselines on 2026-03-30 UTC.
- Shell composition and overall layout: The page keeps the shared authenticated shell, then uses a dedicated review hero plus one curated queue surface so the route reads like the reference's dense decision workspace instead of a stack of generic cards.
- Headline scale and spacing: The hero stays top-heavy, while the queue surface adds a secondary section head that keeps the "Decision Queue" context visible even when entries begin immediately below it.
- CTA treatment and hierarchy: Approve stays the primary gradient action, reject remains visually distinct with its own reason input and softened secondary button treatment, and version-detail navigation now sits in the same immediate action cluster as approval.
- Card and background layering: Pending items are grouped as elevated review objects with softened fact tiles and action rails rather than table chrome or repeated section dividers.
...
- Information density and grouping: Each queue entry now groups identity, version state, summary facts, detail navigation, and approve/reject controls into one horizontal review object so the page remains dense but still scannable.
...
- Intentional deviations and truthful substitutions: Search, filter, history, load-more, and diff tooling remain omitted because the current server-rendered queue only exposes pending-review decisions and detail links; the reference's mock performance and validation facts were replaced with truthful publisher, submission timestamp, and live version-detail access.
- Residual fidelity delta: The current queue still lacks the reference's richer machine-generated diagnostics and avatar/icon treatment because no truthful backend data exists for those elements today.

### `/tenants/:tenantId/agents/:agentId`

- Reference assets: `active_agent_detail/code.html`, `active_agent_detail/screen.png`
- Review evidence: `code.html` and `screen.png` were compared against the extracted `ui/pages/agent-detail.ts` renderer, the shared dossier primitives under `ui/primitives/`, and refreshed approved desktop and mobile baselines on 2026-03-30 UTC after the dossier layout pass.
- Shell composition and overall layout: The shared shell now feeds into a dedicated dossier renderer with a wide hero, staggered publication card list, stacked side panels, and a separate audit-trail section that stacks vertically on smaller viewports.
- Headline scale and spacing: The active-agent headline and pulsed status signal reproduce the premium artifact feel from the reference while the new fact-tile row truthfully replaces the mock KPI cards.
...
- Card and background layering: Publication, overlay, control, and history areas now compose shared `page-hero`, `stat-tile`, `card-head`, `side-panel`, `record-list`, and empty-state primitives instead of one-off inline markup.
...
- Information density and grouping: Active publications, overlay state, environment controls, and version history are separated into focused panels so technical detail remains readable and truthful, including first-class empty states for no approved version, no environment overlays, and no history.
...
- Intentional deviations and truthful substitutions: The reference's deploy-update, maintenance-toggle, add-custom-protocol, and uptime/token KPI elements were replaced with truthful tenant, active-version, publication-count, and overlay-count facts because the current route only supports overlay mutations, publication facts, and dossier navigation.
- Residual fidelity delta: The page now holds the approved dossier proportions for the current implementation, while unsupported utility chrome and synthetic operational metrics remain intentionally absent because the product still cannot expose them truthfully.

### `/tenants/:tenantId/agents/:agentId/versions/:versionId`

- Reference assets: `version_detail/code.html`, `version_detail/screen.png`
- Review evidence: `code.html` and `screen.png` were compared against the updated version-detail route, the shared dossier primitives under `ui/primitives/`, and the approved desktop and mobile baselines on 2026-03-30 UTC.
- Shell composition and overall layout: The shared authenticated shell now leads into a dedicated dossier wrapper with a wide hero, left-column contract and manifest stack, environment sections, telemetry, and health history plus a sticky right-column review-state, audit-history, and supporting-metadata stack that collapses cleanly on mobile.
- Headline scale and spacing: The page keeps the display headline, approval-state framing, and generous editorial spacing from the reference while the new hero stat row gives the route the same top-heavy dossier posture as the approved mock.
- CTA treatment and hierarchy: Submit, approve, and reject remain the only primary actions, grouped inside one explicit action cluster immediately beneath the review-state panel so the right column keeps decision-making prominent.
- Card and background layering: Shared `page-hero`, `section-frame`, `record-list`, `card-head`, `pill`, and `side-panel` primitives now drive the dossier layout, while the manifest keeps the dark technical treatment and the review card adds the stronger editorial emphasis seen in the reference.
...
- Information density and grouping: Review state, action controls, audit history, supporting metadata, publication contracts, manifest data, environment dossiers, telemetry, health history, and review notes are split into distinct dossier panels so dense technical content stays scannable.
...
- Intentional deviations and truthful substitutions: Mock KPI cards, copy utilities, review badges, and fictional side metadata were replaced with truthful contracts, review-state records, audit-history entries, supporting metadata, and read-only manifest output from the current product.
...
| Mock audit-history narrative and side metadata such as license and production-ready environment labels | Audit-history entries and supporting metadata sourced from publisher ownership, card profile, agent and version identifiers, active status, and review timestamps | The current route exposes real review and release metadata but not the reference's fictional editorial side facts. |
